...
async function testSupabaseConnection() {
  ...
  try {
    ...
    const { error: connectionError } = await supabase.from('cards').select('count');
    ...
    const categories = ["light & playful", "self & growth", "life & legacy", "love & relationships", "vulnerability & truths"];
    for (const category of categories) {
      ...
      console.log(`📊 Category "${category}": ${categoryCards?.length || 0} cards`);
    }
