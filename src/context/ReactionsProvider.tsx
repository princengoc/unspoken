...
interface ReactionsContextType {
  ...
  toggleRipple: (toId: string, cardId: string) => Promise<void>;
...
export function ReactionsProvider({
  ...
  const toggleRipple = async (toId: string, cardId: string): Promise<void> => {
    try {
      await reactionsService.toggleRipple(roomId, toId, userId, cardId);
    } catch (error) {
  ...
  const hasTellMeMore = (otherPlayerId: string): boolean => {
  ...
  const isRippled = (toId: string, cardId: string): boolean => {
