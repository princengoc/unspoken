...
const AudioMessagesContext = createContext<AudioMessagesContextType | null>(null);
...
export function AudioMessagesProvider({ roomId, children }: AudioMessagesProviderProps) {
  const { user } = useAuth();
  ...
  const sendAudioMessage = async (audioBlob: Blob, privacy: AudioPrivacy) => {
    ...
    try {
      return await audioMessagesService.uploadAudioMessage(roomId, user.id, audioBlob, privacy);
    } catch (error) {
...
export function useAudioMessages() {
  ...
  if (!context) {
    throw new Error('useAudioMessages must be used within an AudioMessagesProvider');
  }
  return context;
}